...
import {
  ...
  advanceSocraticSession,
  endSocraticSession,
  selectSocraticTargetFromPath,
} from './socratic-service.js';
import { evidencePacks, evidencePackItems, evidenceItems } from './db/schema.js';
...
app.get('/api/learning/report', async (req, res) => {
  ...
  try {
    const [path, assets, decisions] = await Promise.all([
      learningStore.getPathGraph(learner.id),
      learningStore.listAssets(learner.id),
      listDecisions(learner.id, 10),
    ]);

    // 计算知识盲区：掌握度低且未标记为已掌握的节点
    const knowledgeGaps = path.nodes
      .filter((node) => !node.mastered)
      .map((node) => {
        const confidence = node.bktParams?.pMastery ?? 0.5;
        const criticality = node.recommendation === 'critical' ? 1.0 : node.recommendation === 'important' ? 0.7 : 0.4;
        const gapScore = criticality * (1 - confidence);
        return {
          nodeId: node.id,
          title: node.title,
    ...
    // 计算路径进度
    const completed = path.nodes.filter((node) => node.userStatus === 'completed' || node.mastered).length;
    const inProgress = path.nodes.filter((node) => node.userStatus === 'learning').length;
    const pending = path.nodes.filter((node) => node.userStatus === 'pending').length;
    const pathProgress = { completed, inProgress, pending, total: path.nodes.length };

    // 计算当前综合掌握度和推荐难度区间
    const masteryLevels = path.nodes.map((node) => node.bktParams?.pMastery ?? 0.5);
    const currentMastery = masteryLevels.length > 0
      ? masteryLevels.reduce((sum, val) => sum + val, 0) / masteryLevels.length
      : 0.5;

    // 推荐难度区间：当前掌握度 ±0.15，确保适度挑战
    const difficultyMatch = {
      currentMastery,
      recommendedMin: Math.max(0, currentMastery - 0.15),
      recommendedMax: Math.min(1, currentMastery + 0.20),
    };

    // 最近决策记录
    const recentDecisions = decisions.map((d) => ({
      timestamp: d.timestamp,
      agentId: d.metadata?.agentId ?? 'system',
      decision: d.decision,
    ...
    res.status(500).json({
      success: false,
      error: error instanceof Error ? error.message : '学情报告生成失败'
    });
  }
});

// 苏格拉底式引导对话接口
app.post('/api/learning/socratic/start', async (req, res) => {
  const learner = await requireLearner(req, res);
  if (!learner) return;

  try {
    const path = await learningStore.getPathGraph(learner.id);
    const target = selectSocraticTargetFromPath(path.nodes);

    if (!target) {
      res.status(400).json({ success: false, error: '暂时没有需要追问的知识点' });
      return;
    }

    const session = startSocraticSession(learner.id, target.knowledgePointId, target.label, target.confidence);
    const firstQuestion = await generateSocraticQuestion(session);
    ...
    res.status(500).json({ success: false, error: error instanceof Error ? error.message : '启动引导对话失败' });
  }
});

app.post('/api/learning/socratic/answer', async (req, res) => {
  const learner = await requireLearner(req, res);
  if (!learner) return;
  ...
  if (!sessionId || !answer) {
    res.status(400).json({ success: false, error: '缺少会话ID或回答内容' });
    return;
  }

  try {
    const session = getSocraticSession(sessionId);
    ...
    const evaluation = await evaluateSocraticAnswer(session, currentQuestion, answer);
    const { shouldContinue, newConfidence } = advanceSocraticSession(session, currentQuestion, answer, evaluation);

    let nextQuestion: string | null = null;
    if (shouldContinue) {
      nextQuestion = await generateSocraticQuestion(session);
    } else {
      endSocraticSession(sessionId);
    }
    ...
    res.status(500).json({ success: false, error: error instanceof Error ? error.message : '回答评估失败' });
  }
});

app.post('/api/learning/socratic/end', async (req, res) => {
  const learner = await requireLearner(req, res);
  if (!learner) return;

  const sessionId = typeof req.body?.sessionId === 'string' ? req.body.sessionId : '';
  if (!sessionId) {
    res.status(400).json({ success: false, error: '缺少会话ID' });
    return;
  }
...
// 获取资源的证据溯源信息
app.get('/api/learning/evidence/:resourceId', async (req, res) => {
  const learner = await requireLearner(req, res);
  if (!learner) return;

  const resourceId = req.params.resourceId;
  if (!resourceId) {
    res.status(400).json({ success: false, error: '缺少资源ID' });
    return;
  }

  try {
    const resource = (await learningStore.listAssets(learner.id)).find((asset) => asset.id === resourceId);
    if (!resource) {
      res.status(404).json({ success: false, error: '资源不存在' });
      return;
    }

    // 从资源的 evidenceIds 字段查询关联的证据包
    const evidenceIds = resource.evidenceIds ?? [];
    if (evidenceIds.length === 0) {
      res.json({ success: true, evidences: [] });
      return;
    }

    // 从 evidence_packs 表查询证据详情
    const database = getLearningDatabase();
    const evidences = await Promise.all(
      evidenceIds.map(async (packId) => {
        try {
          ...
          const items = await database.db
            ...
            .where(eq(evidencePackItems.packId, packId))
            .limit(10);

          // 获取第一个证据项的详细内容
          const firstItem = items[0];
          let content = '证据内容不可用';
          let sourceType: 'structured' | 'document' | 'web' = 'structured';
          let metadata: Record<string, unknown> = {};

          if (firstItem) {
            const evidenceItem = await database.db
              .select()
              .from(evidenceItems)
              .where(eq(evidenceItems.id, firstItem.evidenceId))
              .limit(1);

            if (evidenceItem.length > 0) {
              const item = evidenceItem[0];
              content = item.content.slice(0, 500); // 限制长度
              sourceType = item.sourceType as 'structured' | 'document' | 'web';
              metadata = {
                datasetName: item.datasetName || 'Unknown',
          ...
          return {
            id: packData.id,
            source: packData.query || '证据包',
            sourceType,
            content,
            confidence: packData.avgConfidence || 0.5,
            validationStatus: 'supported' as const,
            metadata,
          };
        } catch (error) {
          console.error(`查询证据包 ${packId} 失败：`, error);
          return null;
        }
      })
    );
    ...
    // 过滤掉查询失败的证据
    const validEvidences = evidences.filter((e): e is NonNullable<typeof e> => e !== null);

    res.json({ success: true, evidences: validEvidences });
  } catch (error) {
    res.status(500).json({
      success: false,
      error: error instanceof Error ? error.message : '证据查询失败'
    });
  }
});
