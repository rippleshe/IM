...
const TYPE_GUIDES: Record<ResourceGenerationContext['type'], string> = {
  ...
  presentation: [
    ...
    '- 每页写 notes（100 到 200 字本页说明）：只解释本页新增的信息、它与前后页的关系、依据能支持的判断及其边界；用连贯的学习材料语言，不写“讲解词”、教师口吻或泛化的“先看、复述、再继续下一页”口令。只有练习页才说明题目和预期产出。',
    '- 第 2 页必须先用白话解释本次最重要的 2 到 3 个词；后续每页最多引入 2 个新词，字段名必须和中文含义同时出现。',
    '- 只有确实需要操作或判断的页面才安排一个明确动作，并说明动作与本页结论的关系；其他页面专注于把信息和边界讲清楚。每页还要有一个清晰的视觉锚点（术语卡、流程、对照、样本表或练习框），不要让页面退化成文字清单。',
...
function schemaHint(type: ResourceGenerationContext['type']): string {
  ...
  switch (type) {
    ...
    case 'presentation':
      return base + '"slides":[{"heading":"页标题（不超过 16 字）","bullets":["3 到 5 条要点，每条不超过 22 字"],"notes":"100 到 200 字本页说明：解释本页新增信息、与前后页的关系、证据支持的判断和边界；不写教师讲课口吻，不写泛化的复述或翻页口令"}]}';
    case 'tiered_quiz':
