...
- README 增加文档职责边界；提交包的作品介绍、设计实现方案、单元测试说明、评测方案与结果报告、系统部署说明和提交说明同步到 26 个测试文件 / 194 项用例与 2026-09-02 状态。
- 清理源码注释和内部记录中的失效规划引用；升级计划进入回收站，其他用户已有删除状态未处理。
- 回归通过：`pnpm test -- --run`（26 文件 / 194 项）、`pnpm typecheck`、`pnpm lint`、`pnpm --dir web lint`、`pnpm --dir web build`；未发生网络下载或 live 模型调用。
