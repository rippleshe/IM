...
import { dataSource, identityStore, learningStore } from '../server/study-context.js';
...
async function fallbackProfileSnapshot(learnerId: string, persona: PersonaSeed, correct: number, total: number): Promise<void> {
...
async function main(): Promise<void> {
  ...
  console.log(`[demo:seed] 数据源：${dataSource}`);

  for (const persona of PERSONAS) {
    let learnerId: string;
    const existing = await identityStore.getByLoginName(persona.loginName);
    if (existing) {
      learnerId = existing.id;
      await identityStore.resetPassword(learnerId, password);
      console.log(`[demo:seed] = ${persona.loginName} 已存在，密码已同步为当前 IM_TRAINING_AGENT_DEMO_PASSWORD（幂等）`);
    } else {
      const user = await identityStore.register({ loginName: persona.loginName, displayName: persona.displayName, password });
      learnerId = user.id;
      console.log(`[demo:seed] ✔ ${persona.loginName} 注册成功`);
    }
    await identityStore.saveOnboarding(learnerId, persona.onboarding);
    ...
    for (const observation of result.byKnowledgePoint) {
      await learningStore.applySkillObservation(learnerId, observation.knowledgePointId, observation.correct, 'diagnostic_seed');
    }
    await learningStore.saveDiagnosticSession(
      learnerId, result,
    ...
    await learningStore.replacePathGraph(learnerId, pathGraph.nodes, pathGraph.edges);
    ...
    try {
      ...
      await fallbackProfileSnapshot(learnerId, persona, result.correct, result.total);
    }
